...
[English](MCP-SETUP.md) | [简体中文](MCP-SETUP.zh-CN.md) | **繁體中文**

Vigil 是透過 [MCP](https://modelcontextprotocol.io/) 提供的簽名前交易證據面板。agent 準備好一筆 Monad 交易後，Vigil 對主網跑模擬，把實際會發生什麼渲染在對話裡。

三種跑法：**stdio**（本機，最快）、**HTTP**（部署後給 remote connector 用）、**Cloudflare Workers**（線上那份用的就是這個）。
...
`wrangler.toml` 的 `PUBLIC_URL` 要填部署後的實際網址（面板拿它開簽名頁），
...
**改過面板或簽名頁之後，只跑 `pnpm worker:deploy` 是不夠的**——它們是建置時內嵌
進 bundle 的，要先 `pnpm build:worker`，不然推上去的還是舊 HTML。
