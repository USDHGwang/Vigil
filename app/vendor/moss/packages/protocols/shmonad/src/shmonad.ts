...
/**
 * Coverage alone does not make a Receipt trustworthy: claiming every Change
 * says nothing about whether the claim is right. Any contract can emit a log
 * whose signature decodes against this ABI, and an unrelated native transfer
 * can carry a matching value, so provenance is checked before a Change is
 * allowed to describe a shMONAD operation.
 */
function assertShMonadEvent(change: Extract<Change, { kind: "event" }>): void {
  if (!isSameAddress(change.address, SHMONAD_ADDRESS)) {
    throw new Error(`Unexpected Change: ${change.address} is not the shMONAD contract`);
  }
}
...
export class ShMonad {
  ...
  stakeReceipt(changes: readonly Change[]): ReceiptResult<StakeOutcome> {
    ...
    if (!isSameAddress(nativeTransfer.to, SHMONAD_ADDRESS)) {
      throw new Error("shMONAD stake native transfer did not reach the shMONAD contract");
    }
    if (!isSameAddress(nativeTransfer.from, depositEvent.depositor)) {
      throw new Error("shMONAD stake depositor differs between Deposit event and native transfer");
    }
  ...
  unstakeReceipt(changes: readonly Change[]): ReceiptResult<UnstakeOutcome> {
    ...
    if (!isSameAddress(nativeTransfer.from, SHMONAD_ADDRESS)) {
      throw new Error("shMONAD unstake native transfer did not come from the shMONAD contract");
    }
    if (!isSameAddress(nativeTransfer.to, withdrawEvent.receiver)) {
      throw new Error(
        "shMONAD unstake receiver differs between Withdraw event and native transfer",
      );
    }
