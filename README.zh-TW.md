...
你跟 agent 說話
   ↓
agent 呼叫 preview_transaction（MCP tool）
   ↓
Moss 建出未簽交易 → debug_traceCall 打 Monad 主網 → 拿到逐條真實變動
   ↓
人話轉換 · 結構層比對 · 真實餘額檢查
   ↓
面板（MCP App）或文字面板（CLI host）
   ↓
你按簽名 → 交接到簽名頁（指紋對照）→ 你自己的錢包
```

`verifyReceiptCoverage` 檢查報告與變動一一對應：數量相等，且逐筆做物件身分比對，防漏、防重複、防捏造。
